...
from networks.Fusing.resnet_PSM import resnet50
        ...
        with torch.no_grad():
            ...
            for type_no in range(self.score_filter_type_size):
                score_avg = nn.functional.avg_pool2d(fm, self.score_filter_size_list[type_no],
                                                     stride=1)  # (7,2048,5,5), (7,2048,6,6) 这里做了一个特征池化，应该就是计算了每个patch的分数
                score_sum = torch.sum(score_avg, dim=1,
                                      keepdim=True)  # (7,1,5,5), (7,1,6,6) #since the last operation in layer 4 of the resnet50 is relu, thus the score_sum are greater than zero
            ...
            for proposal_no in range(proposal_size):
                ...
                if patch_height == 224 and patch_width == 224:
                    ...
                    window_imgs[batch_no, proposal_no:proposal_no + 1] = F.interpolate(img_patch[None, ...],
                               size=(224, 224), mode='bilinear', align_corners=True)  # [N, 4, 3, 224, 224]
