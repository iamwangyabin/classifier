...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        self.compound_prompts_text = nn.ParameterList(
            [nn.Parameter(torch.empty(self.text_encoder_n_ctx, ctx_dim)) for _ in
             range(self.compound_prompts_depth - 1)])
    ...
    def forward(self, cls_id=None):
        ...
        prompts_pos = torch.cat(  # N(the number of template), 1, dim
            [
