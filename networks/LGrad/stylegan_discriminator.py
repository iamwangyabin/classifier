...
class ConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if use_wscale:
            ...
            self.wscale = (wscale * lr_mul).item()
        else:
            self.weight = nn.Parameter(torch.randn(*weight_shape) * wscale / lr_mul)
            # self.weight = nn.Parameter(torch.randn(*weight_shape))
            self.wscale = torch.tensor(lr_mul, dtype=self.weight.dtype, requires_grad=True)

        if add_bias:
            self.bias = nn.Parameter(torch.zeros(out_channels))
            self.bscale = torch.tensor(lr_mul, dtype=self.bias.dtype, requires_grad=True)
        else:
            self.bias = None
...
#     def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, wscale=1.0):
#         super(ScaledConv2d, self).__init__()
#         self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
#         self.weight = nn.Parameter(torch.randn(self.conv.weight.shape))
#         self.wscale = wscale
#     def forward(self, x):
...
# conv_layer = ScaledConv2d(3, 16, 3, wscale=0.5)
# output = conv_layer(x)
# print(output.shape)